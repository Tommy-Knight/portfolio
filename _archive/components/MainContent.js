		...
		<main className='content-container show'>
			...
			<div className='content-cards' ref={cardsRef}>
				<AboutCard
					...
					onNavClick={handleCardNavClick}
					ref={(el) => (cardElements.current['about'] = el)}
				/>
				<ProjectsCard
					isActive={activeCard === 'projects'}
					onNavClick={handleCardNavClick}
					ref={(el) => (cardElements.current['projects'] = el)}
				/>
				<CvCard
					isActive={activeCard === 'cv'}
					onNavClick={handleCardNavClick}
					ref={(el) => (cardElements.current['cv'] = el)}
				/>
				<ContactCard
					isActive={activeCard === 'contact'}
					onNavClick={handleCardNavClick}
					ref={(el) => (cardElements.current['contact'] = el)}
				/>
			</div>
		</main>
	);
